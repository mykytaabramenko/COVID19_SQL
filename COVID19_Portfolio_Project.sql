...
SELECT 
  ...
  )* 100 AS Percent_Fully_Vaccinated 
FROM 
  Vaccination 
WHERE 
  location = 'Ukraine';
...
SELECT 
  ...
  Cases.total_deaths, 
  (
    Cases.total_deaths / Cases.total_cases
  )* 100 AS deaths_to_cases_percent, 
  (
    Cases.total_deaths / Cases.population
  )* 100 AS deaths_to_pop_percent, 
  (
    Cases.total_cases / Cases.population
  )* 100 AS cases_to_pop_percent, 
  vac.new_vaccinations, 
  vac.people_vaccinated, 
  vac.people_fully_vaccinated, 
  (
    vac.people_vaccinated / vac.population
  )* 100 as Percent_Vaccinated, 
  (
    vac.people_fully_vaccinated / vac.population
  )* 100 as Percent_Fully_Vaccinated 
FROM 
  Cases 
  INNER JOIN Vaccination vac ON Cases.date = vac.date 
  AND Cases.location = vac.location 
WHERE 
  Cases.location = 'Ukraine';
-- It may be interesting to visualize this data later, so let's create a view
CREATE VIEW CasesVsVaccin AS (
  SELECT 
    Cases.location, 
    Cases.date, 
    Cases.total_cases, 
    ...
    (
      Cases.total_deaths / Cases.population
    )* 100 as Country_Death_Rate, 
    (
      Cases.total_cases / Cases.population
    )* 100 as Percent_of_people_got_covid, 
    vac.new_vaccinations, 
    vac.people_vaccinated, 
    vac.people_fully_vaccinated, 
    (
...
SELECT 
  continent, 
  SUM(
    CAST(new_cases AS BIGINT)
  ) AS total_cases, 
  SUM(
    CAST(new_deaths AS BIGINT)
  ) AS total_deaths 
FROM 
  Cases 
WHERE 
...
GROUP BY 
  continent 
ORDER BY 
  total_cases -- Let's add percentage measurements to this data just how we have done this earlier (but without breaking things down by date). 
  ...
  WITH Continents AS (
    SELECT 
      ...
      SUM(
        CAST(
          new_cases AS DECIMAL(11, 0)
        )
      ) AS total_cases_by_continent, 
      SUM(
        CAST(
          new_deaths AS DECIMAL(11, 0)
        )
      ) AS total_deaths_by_continent 
    FROM 
      Cases 
    WHERE 
      continent IS NOT NULL 
...
SELECT 
  ...
  (
    total_deaths_by_continent / total_cases_by_continent
  )* 100 AS Deaths_To_Cases_Percentage 
FROM 
  Continents 
ORDER BY 
  Deaths_To_Cases_Percentage -- Let's also look at the ration of total deaths and total cases to overall continent population
...
GROUP BY 
  continent, 
  location, 
  population -- Let's now sum the population by continent
SELECT 
  continent, 
...
GROUP BY 
  continent -- Finally, let's join these table with the table we created earlier
SELECT 
  Cases.continent, 
  SUM(
    CAST(Cases.new_cases as NUMERIC)
  ) AS total_cases_by_continent, 
  SUM(
    CAST(Cases.new_deaths AS NUMERIC)
  ) As total_deaths_by_continent, 
...
FROM 
  ...
  ) population_by_continent ON population_by_continent.continent = Cases.continent 
GROUP BY 
  Cases.continent, 
  population_by_continent.Population -- Now let's use CTE to count the ratio of deaths to population, cases to population, cases to deaths in percents
...
SELECT 
  ...
  (
    total_deaths_by_continent / Population
  )* 100 AS Deaths_To_Pop_Percent, 
  (
    total_cases_by_continent / Population
  )* 100 AS Cases_To_Pop_Percent 
FROM 
  continents_deaths_cases_pop 
ORDER BY 
  Cases_To_Pop_Percent -- Let's now create a temporary table with the data on cases, deaths, population
DROP 
  TABLE IF EXISTS #Data_By_Continents
...
WITH continents_deaths_cases_pop AS (
  SELECT 
    ...
    SUM(
      CAST(
        Cases.new_cases AS DECIMAL(15, 2)
      )
    ) AS total_cases_by_continent, 
    SUM(
      CAST(
        Cases.new_deaths AS DECIMAL(15, 2)
      )
    ) AS total_deaths_by_continent, 
...
FROM 
  ...
  INNER JOIN (
    ...
    GROUP BY 
      continent
  ) density_gdp_hum_dev_by_cont ON density_gdp_hum_dev_by_cont.continent = #Data_By_continents.continent
...
SELECT 
  ...
  COALESCE(
    ...
    0
  ) AS People_vaccinated -- MAX because the number of people vaccinated by the time the data was downloaded is the maximum number in people_vaccinated column in particular country
FROM 
  Vaccination 
WHERE 
  continent IS NOT NULL 
GROUP BY 
  continent, 
  location --- Let's break than down by continent
SELECT 
  continent, 
...
GROUP BY 
  continent -- Let's now join this table to the table we promised to refer to
SELECT 
...
FROM 
  ...
  INNER JOIN (
    ...
    GROUP BY 
      continent
  ) vac_by_cont ON vac_by_cont.continent = cases_deaths_gdp_hum_dev_pop_dens_by_continent.continent -- Let's create a new (now permanent) table to store this data
DROP 
  TABLE IF EXISTS data_by_continent CREATE TABLE data_by_continent(
    continent VARCHAR(20), 
...
FROM 
  ...
  INNER JOIN (
    ...
    GROUP BY 
      continent
  ) vac_by_cont ON vac_by_cont.continent = cases_deaths_gdp_hum_dev_pop_dens_by_continent.continent -- Now let's add the column of the percent ratio of people vaccinated to the whole population
SELECT 
  *, 
...
-- Let's now deop the temporary table we created - we don't need it anymore
DROP 
  TABLE #Data_By_continents
...
DROP 
  TABLE IF EXISTS Human_development_top;
...
SELECT 
  * 
FROM 
  Human_development_top -- Let's do the same with gdp per capita
DROP 
  TABLE IF EXISTS gdp_top;
...
SELECT 
  * 
FROM 
  gdp_top -- Now let's extract the  new cases, total cases, new deaths, total deaths, new vaccinations, total vaccinations for these countries
...
FROM 
  Cases 
  INNER JOIN Vaccination vac ON Cases.location = vac.location 
